...
function drawElevation(
    ...
    if (isGable && ridgeH > 0) {
        if (isRidgeView(direction)) {
            ...
            drawBadge(ctx, bx + bw / 2, by, `${pitchDegR}°  ${spanMmR}mm`);
        } else {
            ctx.beginPath();
            ctx.moveTo(bx, wallTop);
            ctx.lineTo(bx, by);
    ...
    return (
        <div className="elev-panel">
            ...
            <div className="elev-body">
                <div className="elev-inputs">

                    <label className="elev-input-label">
                        <span>{wallH.toFixed(1)} m</span>
                        <span className="elev-input-sub">Wall height</span>
                        <input
                            type="range" min={0.5} max={15} step={0.1}
                            value={wallH}
                            className="elev-slider"
                            onChange={(e) => {
                                if (!building) return;
                                updateBuildingProps(building.id, { roofHeight: parseFloat(e.target.value) });
                            }}
                        />
                    </label>
                    {building?.roofType === 'gable' && (
                        <label className="elev-input-label">
                            <span>{ridgeH.toFixed(1)} m</span>
                            <span className="elev-input-sub">Ridge height</span>
                            <input
                                ...
                                onChange={(e) => {
                                    if (!building) return;
                                    updateBuildingProps(building.id, { ridgeHeight: parseFloat(e.target.value) });
                                }}
                            />
                        </label>
                    )}
